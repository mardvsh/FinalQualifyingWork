...
detector = cv2.dnn.readNetFromCaffe(prototxt=prototxt_path, caffeModel=model_path)
...
def main():
    #video_source = r"resources/2.mp4"
    video_source = 0
    ...
    loitering_threshold = 5  # Пороговое время пребывания (в секундах) для отображения надписи "Loitering!"
    ...
    while True:
        ...
        for (objectId, bbox) in objects.items():
            ...
            if dwell_time[objectId] > loitering_threshold:
                text_loitering = "Loitering!"
                cv2.putText(frame, text_loitering, (x1, y1 - 25), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 1)
